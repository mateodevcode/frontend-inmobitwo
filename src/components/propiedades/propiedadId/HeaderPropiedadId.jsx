...
const HeaderPropiedadId = () => {
  const { usuario, openModalUserPropiedadId, setModalUserPropiedadId } =
    useAppContext();
  const { name } = usuario;
  ...
  return (
    <div className="border-black/20 border-b flex justify-end items-center gap-3 p-2 px-4">
      ...
      {/* usuario */}
      <div
        className="w-10 h-10 bg-gray-200 hover:bg-gray-300 rounded-full relative cursor-pointer select-none active:scale-95 duration-300 transition flex items-center justify-center"
        onClick={() => setModalUserPropiedadId(!openModalUserPropiedadId)}
      >
        <div className="text-gray-800 font-semibold flex items-center justify-center">
          {getInitials(name)}
        </div>
